...
def test_main_help(runner):
    ...
    assert 'CLI tool to interact with Grok LLM.' in result.output, "Help text should include CLI description"
...
def test_init_command(runner, tmp_path, monkeypatch):
    ...
    assert Path('.grkrc').exists(), "Init command should create .grkrc file"
    assert 'Default .grkrc with profiles created successfully' in result.output, "Init should confirm file creation"
...
def test_run_command_no_api_key(runner, tmp_path, monkeypatch):
    ...
    assert result.exit_code != 0, "Run command should fail without API key"
    assert 'API key is required' in result.output, "Error message should mention missing API key"
...
def test_run_command_file_not_found(runner, tmp_path, monkeypatch):
    ...
    assert result.exit_code != 0, "Run command should fail for non-existent file"
    assert 'Failed to read file' in result.output, "Error message should mention file read failure"
...
def test_run_command_with_profile(runner, tmp_path, monkeypatch, profile, mocker):
    ...
    assert result.exit_code == 0, f"Run command should succeed with profile {profile}"
    assert f"Running grk with profile '{profile}'" in result.output, f"Output should mention profile {profile}"
    assert Path('output.txt').exists(), "Output file should be created"
...
def test_call_grok_api_failure(runner, tmp_path, monkeypatch, mocker):
    ...
    assert "API request failed" in str(exc_info.value), "Should raise ClickException for API failure"
